...
void CreateWave(){
    ...
    float randomAmplitude = GetRandomValue(guiSettings.minAmplitude, guiSettings.maxAmplitude);
    float randomFrequency = GetRandomValue(guiSettings.minFrequency, guiSettings.maxFrequency);
    float randomPhase = GetRandomValue(guiSettings.minPhase, guiSettings.maxPhase);
    std::unique_ptr<SineWave> waveToBeAdded = std::make_unique<SineWave>(randomAmplitude, randomFrequency, randomPhase, 0, randomColor);
...
void RunProgram(){
    ...
    while (!WindowShouldClose())
    {
        ...
        if (!waves.empty()) {
            ...
            //amplitude
            ImGui::SliderFloat("Amplitude", &currentWave->Amplitude(), guiSettings.minAmplitude, guiSettings.maxAmplitude, "%.1f");
            //frequency
            ImGui::SliderFloat("Frequency", &currentWave->Frequency(), guiSettings.minFrequency, guiSettings.maxFrequency, "%.1f", ImGuiSliderFlags_Logarithmic);
            //phase
            float phase = currentWave->Phase();
            if (ImGuiKnobs::Knob("Phase", &phase, guiSettings.minPhase, guiSettings.maxPhase, 15.0f, "%.1f deg", ImGuiKnobFlags_ValueTooltip)) {
                currentWave->Phase() = phase;
            }
            ImGui::SliderFloat("Phase Speed", &currentWave->PhaseSpeed(), guiSettings.minPhaseSpeed, guiSettings.maxPhaseSpeed, "%.1f" ,ImGuiSliderFlags_Logarithmic);
            //color
