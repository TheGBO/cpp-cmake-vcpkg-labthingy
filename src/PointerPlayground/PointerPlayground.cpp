...
int RunPointerPlayground(){
    ...
    {
        fmt::println("address: {} | value(hex): 0x{:02x} | value(int): {:03} | value(txt) {:c}", (void*)thePointer, *thePointer, *thePointer, *thePointer);
        thePointer++;
    }
    ...
    fmt::println("Ok now I shall increment the derefenwughwgjced b just by a wee bit :)))))");
    (*b)++;
    fmt::println(strTemplate, a, (void*)b, *b);
    return 0;
}